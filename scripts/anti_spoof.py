...
class HistoryManager:
    ...
    def add_carrier_observation(
        self, phone: str, carrier: str | None, line_type: str | None
    ) -> None:
    ...
    def update_behavioral_profile(self, phone: str, message: str) -> None:
        ...
        if emojis:
            ...
            profile["emoji_count"] = (
                profile["emoji_count"] * (profile["message_count"] - 1) + len(emojis)
        ...
        if message:
            ...
            if alpha_chars:
                ...
                profile["caps_ratio"] = (
                    profile["caps_ratio"] * (profile["message_count"] - 1) + caps_ratio
                ) / profile["message_count"]
        ...
        if re.search(pattern, message_lower):
            indicators.append(
                SpoofIndicator(
                    type=IndicatorType.BEHAVIORAL,
                    severity=Severity.HIGH,
                    detail="Suspicious content: possible credential/sensitive data request",
                    confidence=0.8,
                )
            )
            break
        ...
        if re.search(pattern, message_lower):
            indicators.append(
                SpoofIndicator(
                    type=IndicatorType.BEHAVIORAL,
                    severity=Severity.HIGH,
                    detail="Social engineering pattern detected",
                    confidence=0.85,
                )
            )
            break
