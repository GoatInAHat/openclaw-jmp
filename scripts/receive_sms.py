...
class SMSReceiver:
    ...
    def __init__(
        self, credentials_path: Path = CREDENTIALS_PATH, callback=None, once: bool = False
    ):
...
async def main_async(args):
    """Async main function."""
    receiver = SMSReceiver(credentials_path=args.credentials, once=args.once)
...
def main():
    ...
    parser.add_argument("--once", action="store_true", help="Exit after receiving one message")
    parser.add_argument(
        "--timeout", "-t", type=float, default=0, help="Timeout in seconds (0 = run forever)"
    )
