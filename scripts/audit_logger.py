...
class AuditEvent:
    ...
    spoof_indicators: Optional[list] = None
    message_preview: Optional[str] = None  # First 50 chars only
...
class AuditLogger:
    ...
    DEFAULT_LOG_DIR = Path('/data/workspace/skills/jmp-sms/data/audit')
    RETENTION_DAYS = 90
    BUFFER_SIZE = 100
                        ...
                        with opener(path, 'rt') as f:
                            for line in f:
                                ...
                                try:
                                    ...
                                    # Apply filters
                                    if event_types:
                                        if event.get('event_type') not in [e.value for e in event_types]:
                                            continue
