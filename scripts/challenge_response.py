...
def _load_pending_challenges() -> dict[str, Any]:
    ...
    try:
        with open(PENDING_CHALLENGES_PATH) as f:
            ...
            now = time.time()
            data["challenges"] = {
                k: v for k, v in data.get("challenges", {}).items() if v.get("expires_at", 0) > now
            }
            return data
...
async def send_challenge(
    phone: str, reason: str | None = None, spoof_indicators: list[str] | None = None
) -> tuple[bool, str, str]:
    """
    ...
    existing = [v for k, v in data.get("challenges", {}).items() if v.get("phone") == phone]
    if len(existing) >= MAX_PENDING_CHALLENGES_PER_PHONE:
        _log_challenge_event("challenge_limit_exceeded", phone, {"pending_count": len(existing)})
        return (
            False,
            "Too many pending challenges. Please wait for existing challenges to expire.",
            "",
        )
    ...
    try:
        ...
        if not success:
            _log_challenge_event("challenge_sms_failed", phone, {"challenge_id": challenge_id})
...
def verify_challenge(phone: str, response: str) -> tuple[bool, str, str | None]:
    """
    ...
    if not matching_challenges:
        _log_challenge_event("no_pending_challenge", phone, {"response": response})
    ...
    for challenge_id, challenge in matching_challenges:
        ...
        if secrets.compare_digest(response, expected):
            ...
            _log_challenge_event("challenge_verified", phone, {"challenge_id": challenge_id})
...
def get_pending_challenge(phone: str) -> dict[str, Any] | None:
    """
...
def cancel_challenge(challenge_id: str) -> bool:
    ...
    _log_challenge_event("challenge_cancelled", phone or "unknown", {"challenge_id": challenge_id})
...
async def challenge_and_wait(
    phone: str, timeout_seconds: int = CHALLENGE_EXPIRY_SECONDS, reason: str | None = None
) -> tuple[bool, str]:
    """
    ...
    return (
        True,
        f"Challenge sent. ID: {challenge_id}. Awaiting response within {timeout_seconds} seconds.",
    )
