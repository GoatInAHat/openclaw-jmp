...
from audit_logger import AuditLogger, AuditEventType, get_audit_logger, set_audit_logger
from rate_limiter import RateLimiter, RateLimitConfig, get_rate_limiter, set_rate_limiter
...
class SMSDaemon:
    ...
    CONFIG_PATH = Path('/data/workspace/skills/jmp-sms/config/daemon.json')
    ...
    def __init__(
        ...
        self.jmp_client: Optional[JMPClient] = None
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            rate_config = RateLimitConfig.load(
                Path('/data/workspace/skills/jmp-sms/config/rate_limits.json')
            )
            ...
            main_interface = MainAgentInterface(
                send_to_main=self._send_to_main_agent,
...
Examples:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help='Path to daemon configuration JSON file'
    )
    parser.add_argument(
        '--test',
        action='store_true',
        help='Run in test mode with timeout'
    )
    parser.add_argument(
        '--timeout', '-t',
        type=float,
        default=30,
        help='Timeout in seconds for test mode'
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('/data/workspace/skills/jmp-sms/data/daemon.log')
        ]
    )
    ...
    Path('/data/workspace/skills/jmp-sms/data').mkdir(parents=True, exist_ok=True)
