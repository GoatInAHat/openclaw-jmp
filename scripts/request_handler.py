...
class HandlerConfig:
    ...
    # Paths
    audit_log_path: Path = field(default_factory=lambda: Path("/data/workspace/skills/jmp-sms/logs/audit.jsonl"))
    public_info_path: Path = field(default_factory=lambda: Path("/data/workspace/skills/jmp-sms/config/public_info.json"))
...
class GetPublicInfoHandler(RequestHandlerBase):
    ...
    def handle(self, request: GetPublicInfoRequest, metadata: RequestMetadata) -> MainResponse:
        ...
        return self._create_success(
            info,
            internal_note=f"Provided public info for topic: {request.topic.value}"
        )
...
class RequestVerificationHandler(RequestHandlerBase):
    ...
    def handle(self, request: RequestVerificationRequest, metadata: RequestMetadata) -> MainResponse:
...
class EndConversationHandler(RequestHandlerBase):
    ...
    def handle(self, request: EndConversationRequest, metadata: RequestMetadata) -> MainResponse:
        ...
        return EndConversationResponse(
            ...
            ) if request.reason.value == "resolved" else FollowUp(
                type=FollowUpType.OWNER_WILL_CONTACT,
                timeframe="within 24 hours"
            ) if request.reason.value == "escalated" else None,
        )
...
class RequestHandler:
    ...
    def __init__(self, config: HandlerConfig | None = None):
        ...
        self._handlers: dict[str, RequestHandlerBase] = {
            ...
            "check_availability": CheckAvailabilityHandler(self.config, self.audit, self.public_info),
            "request_callback": RequestCallbackHandler(self.config, self.audit, self.public_info),
            "request_appointment": RequestAppointmentHandler(self.config, self.audit, self.public_info),
            ...
            "request_verification": RequestVerificationHandler(self.config, self.audit, self.public_info),
    ...
    def _handle_suspicious(self, request: MiddlemanRequest, metadata: RequestMetadata) -> MainResponse:
    ...
    def _handle_suspicious_owner(self, request: MiddlemanRequest, metadata: RequestMetadata) -> MainResponse:
