...
class RequestMetadata(BaseModel):
    ...
    request_id: str = Field(
        description="UUID for tracking this specific request"
    )
    timestamp: str = Field(
        description="ISO 8601 timestamp when request was created"
    )
    sender_phone: str = Field(
        description="E.164 formatted phone number of sender"
    )
    trust_level: TrustLevel = Field(
        description="Trust level assigned by routing engine"
    )
    conversation_id: str = Field(
        description="Groups messages in same conversation"
    )
    ...
    security_flags: list[str] = Field(
        ...
        description="Security flags from Quarantine Agent"
    )
...
class GetPublicInfoRequest(BaseModel):
    ...
    topic: PublicInfoTopic = Field(
        description="The specific topic to get information about"
    )
...
class CheckAvailabilityRequest(BaseModel):
    ...
    date_range_start: str = Field(
        description="ISO 8601 date for start of range"
    )
    date_range_end: str = Field(
        description="ISO 8601 date for end of range"
    )
    purpose: str | None = Field(
        default=None,
        max_length=500,
        description="Purpose for checking availability"
    )
    ...
    def validate_date(cls, v: str) -> str:
        ...
        try:
            ...
            raise ValueError("Must be valid ISO 8601 date")
        return v
...
class RequestCallbackRequest(BaseModel):
    ...
    preferred_time: str | None = Field(
        default=None,
        max_length=100,
        description="Preferred time for callback (free text)"
    )
...
class RequestAppointmentRequest(BaseModel):
    ...
    preferred_date: str = Field(
        description="Preferred date (ISO 8601 or description)"
    )
    preferred_time: str | None = Field(
        default=None,
        max_length=100,
...
class LeaveMessageRequest(BaseModel):
    ...
    callback_requested: bool = Field(
        default=False,
        description="Whether sender wants a callback"
    )
...
class RelayToOwnerRequest(BaseModel):
    ...
    summary: str = Field(
        min_length=1,
        max_length=1000,
        description="Quarantine's summary of the request"
    )
    ...
    suggested_action: str | None = Field(
        default=None,
        max_length=500,
        description="Suggested action for owner"
    )
...
class EndConversationRequest(BaseModel):
    ...
    reason: EndConversationReason = Field(
...
class CannotProcessRequest(BaseModel):
    ...
    reason: str = Field(
        min_length=1,
        max_length=500,
        description="Why the request cannot be processed"
    )
...
class SuccessResponse(BaseModel):
    ...
    public_message: str = Field(
        min_length=1,
        max_length=2000,
        description="Safe message to send to external party"
    )
    internal_note: str | None = Field(
        default=None,
        max_length=1000,
        description="For logging only - NOT sent to external"
    )
...
class DeniedResponse(BaseModel):
    ...
    public_message: str = Field(
        min_length=1,
        max_length=2000,
        description="What to tell external party"
    )
...
class EscalateResponse(BaseModel):
    ...
    public_message: str = Field(
        min_length=1,
        max_length=2000,
        description="What to tell external party while waiting"
    )
...
class VerificationRequiredResponse(BaseModel):
    ...
    verification_id: str = Field(
        description="ID to track verification flow"
    )
    ...
    def validate_verification_id(cls, v: str) -> str:
        ...
        try:
            ...
            raise ValueError("verification_id must be a valid UUID")
        return v
...
class QuarantineOutput(BaseModel):
    ...
    request: MiddlemanRequest = Field(
        description="The typed request to send to Main Agent"
    )
    response: str = Field(
        min_length=1,
        max_length=2000,
        description="Text to send back to the external person"
    )
    flags: list[str] = Field(
        default_factory=list,
        description="Optional security flags"
    )
...
class MiddlemanMessage(BaseModel):
    ...
    quarantine_response: str = Field(
        description="What Quarantine plans to tell the user"
    )
    flags: list[str] = Field(
        default_factory=list,
        description="Security flags from Quarantine"
    )
