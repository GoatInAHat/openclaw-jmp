...
# Paths
CONFIG_DIR = Path('/data/workspace/skills/jmp-sms/config')
...
class RouteAction(str, Enum):
    ...
    MAIN_AGENT = 'main_agent'        # Route to main agent (Berry)
...
def route_message(sms: InboundSMS) -> RouteDecision:
    ...
    if is_blocked(sender):
        ...
        log_audit_event('blocked_message', sender, TrustLevel.BLOCKED, {
            'action': 'drop',
        })
    ...
    if is_verified_owner(sender):
        ...
        if spoof_result.passed:
            ...
            log_audit_event('routing_decision', sender, TrustLevel.OWNER_VERIFIED, {
                'action': 'main_agent',
            ...
            log_audit_event('spoof_detected', sender, TrustLevel.OWNER_SUSPICIOUS, {
                'action': 'quarantine',
    ...
    if is_known_contact(sender):
        ...
        log_audit_event('routing_decision', sender, TrustLevel.KNOWN_CONTACT, {
            'action': 'quarantine',
        })
        return RouteDecision(
            action=RouteAction.QUARANTINE_AGENT,
    ...
    log_audit_event('routing_decision', sender, TrustLevel.UNKNOWN, {
        'action': 'quarantine',
    })
    return RouteDecision(
        action=RouteAction.QUARANTINE_AGENT,
