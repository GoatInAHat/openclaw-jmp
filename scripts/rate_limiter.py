...
class SlidingWindowCounter:
    ...
    def check_and_record(self, now: float | None = None) -> tuple[bool, int]:
    ...
    def time_until_allowed(self, now: float | None = None) -> int:
...
class RateLimiter:
    ...
    CONFIG_PATH = CONFIG_DIR / "rate_limits.json"
    ...
    def _init_global_counters(self) -> None:
        ...
        self._global_counters = {
            ...
            "quarantine_hour": SlidingWindowCounter(
                3600, self.config.global_quarantine_calls_per_hour
            ),
        }
        ...
        with self._lock:
            if phone not in self._number_counters:
                self._number_counters[phone] = {
                    ...
                    "quarantine": SlidingWindowCounter(
                        3600, self.config.quarantine_requests_per_hour
                    ),
                    "spoof_challenge": SlidingWindowCounter(
                        86400, self.config.spoof_challenges_per_day
                    ),
                }
