...
try:
    from .paths import DATA_DIR, SKILL_DIR
...
class QuarantineResult:
    ...
    def to_middleman_message(self, metadata: RequestMetadata) -> MiddlemanMessage:
...
def load_quarantine_prompt() -> str:
    ...
    if not QUARANTINE_PROMPT_PATH.exists():
        raise QuarantineError(f"Quarantine prompt not found: {QUARANTINE_PROMPT_PATH}")
...
def build_user_message(sms: InboundSMS) -> str:
    ...
    if sms.media:
        lines.extend(
            ["", "## Attachments", f"({len(sms.media)} media attachment(s) - content not shown)"]
        )

    lines.extend(["", "---", "", "Process this message and respond with valid JSON only."])
...
def extract_json_from_response(response: str) -> dict:
    ...
    raise QuarantineValidationError(
        "Could not extract valid JSON from agent response", raw_output=response
    )
...
def invoke_quarantine_agent_cli(
    ...
    """
    if os.environ.get("JMP_ALLOW_OPENCLAW_CLI_QUARANTINE") != "1":
        raise QuarantineAgentError(
            "OpenClaw CLI quarantine is disabled by default because tool isolation "
            "cannot be guaranteed. Use the direct API mode or explicitly set "
            "JMP_ALLOW_OPENCLAW_CLI_QUARANTINE=1 after reviewing your sandbox policy."
        )
    ...
    try:
        result = subprocess.run(
            [
                ...
                "--session-id",
                f"quarantine-{conversation_id}",
                "--thinking",
                QUARANTINE_MODEL_CONFIG["thinking"],
                "--message",
                full_message,
                "--json",
            ],
        ...
        raise QuarantineAgentError("Quarantine agent timed out after 120 seconds") from exc
    except FileNotFoundError as exc:
        raise QuarantineAgentError("OpenClaw CLI not found - is it installed?") from exc
...
def invoke_quarantine_agent_direct(
    ...
    raise QuarantineAgentError("No API key found. Set ANTHROPIC_API_KEY or OPENAI_API_KEY")
...
def _call_anthropic_api(
    ...
    payload = {
        "model": os.environ.get("JMP_ANTHROPIC_MODEL", "claude-sonnet-4-20250514"),
...
def _call_openai_api(
    ...
    payload = {
        "model": os.environ.get("JMP_OPENAI_MODEL", "gpt-4.1-mini"),
        "max_tokens": QUARANTINE_MODEL_CONFIG["max_tokens"],
        "temperature": QUARANTINE_MODEL_CONFIG["temperature"],
        "messages": [
...
def invoke_quarantine_agent(
    ...
    Args:
        ...
        method: 'api' (safe default), 'cli', or 'auto'
...
def process_quarantine_sms(
    ...
    Args:
        ...
        invocation_method: Quarantine backend; direct API is the safe default
    ...
    try:
        ...
        raw_response = invoke_quarantine_agent(
            user_message, conversation_id, method=invocation_method
        )

        # Extract and validate JSON
        logger.debug("Quarantine agent returned %d characters", len(raw_response))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Process an SMS through the Quarantine Agent")
    ...
    parser.add_argument(
        "--method", default="api", choices=["cli", "api", "auto"], help="Invocation method"
    )
    ...
    result = process_quarantine_sms(sms, trust_level, invocation_method=args.method)
