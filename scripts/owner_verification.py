...
def _load_pending_verifications() -> Dict[str, Any]:
    ...
    try:
        with open(PENDING_VERIFICATIONS_PATH) as f:
            ...
            now = time.time()
            data['pending'] = {
                k: v for k, v in data.get('pending', {}).items()
...
async def initiate_verification(
    ...
    if not is_allowed:
        _log_verification_event('rate_limited', phone, {
            'trusted_channel_id': trusted_channel_id
        })
        return False, f"Rate limit exceeded. Max {MAX_ATTEMPTS_PER_HOUR} verification attempts per hour."
    
    # Check if already verified
    owners = _load_verified_owners()
    if phone in owners.get('owners', {}):
        return False, f"Phone number {phone} is already verified. Use remove_verified_owner first if you need to re-verify."
    ...
    _log_verification_event('verification_initiated', phone, {
        'trusted_channel_id': trusted_channel_id,
        'carrier_info': carrier_info,
    ...
    if not success:
        _log_verification_event('sms_send_failed', phone, {
            'trusted_channel_id': trusted_channel_id
        })
        return False, "Failed to send verification SMS. Please try again."
    
    _log_verification_event('otp_sent', phone, {
        'trusted_channel_id': trusted_channel_id
    })
    
    return True, f"Verification code sent to {phone}. Code expires in 10 minutes. Enter the code in this trusted channel to complete verification."
...
def verify_otp(phone: str, code: str) -> Tuple[bool, str]:
    ...
    if not pending:
        ...
        return False, "No pending verification for this phone number. Please initiate verification first."
    ...
    if time.time() > pending.get('expires_at', 0):
        ...
        _save_pending_verifications(data)
        _log_verification_event('verification_expired', phone, {
    ...
    if pending['attempts'] > 3:
        del data['pending'][phone]
        _save_pending_verifications(data)
        _log_verification_event('too_many_attempts', phone, {
            'trusted_channel_id': pending.get('trusted_channel_id'),
    ...
    if not secrets.compare_digest(code, pending.get('otp', '')):
        _log_verification_event('invalid_otp', phone, {
            'trusted_channel_id': pending.get('trusted_channel_id'),
            'attempts': pending['attempts']
        })
    ...
    _log_verification_event('verification_success', phone, {
        'trusted_channel_id': pending.get('trusted_channel_id'),
...
def get_verified_owner(phone: str) -> Optional[Dict[str, Any]]:
