...
from api_schema import (
    ...
    VALID_REQUEST_TYPES,
    TrustLevel as RealTrustLevel,
    RequestMetadata as RealRequestMetadata,
    QuarantineOutput as RealQuarantineOutput,
    LeaveMessageRequest,
    RelayToOwnerRequest,
...
from routing_engine import (
    ...
    TrustLevel as RoutingTrustLevel,
)
...
class InboundSMS:
    ...
    sender: str
    recipient: str = "+13233054987"  # Berry's JMP number
    body: str = ""
...
class QuarantineOutput:
    ...
    request: Optional[MiddlemanRequest] = None
    response: str = ""
...
class QuarantineAgent:
    ...
    def _load_injection_patterns(self) -> List[str]:
        ...
        return [
            ...
            r"this\s+is\s+(really|actually)\s+(you|me|bennett)",
            r"elevated\s+mode",
            r"unrestricted",
    ...
    def process(self, sms: InboundSMS) -> QuarantineOutput:
        ...
        personal_info_patterns = [
            r"password", r"credential", r"api.?key", r"secret",
            r"(his|her|their|bennett'?s).*(schedule|location|email|phone|address)",
            r"where is (he|she|bennett)",
        ...
        if re.search(r"\$\d+|money|payment|bank|account|transfer|funds", body_lower, re.IGNORECASE):
...
class AntiSpoofChecker:
    ...
    def check(self, sms: InboundSMS) -> Dict:
        ...
        confidence = max(0, 1 - (high_severity_count * 0.35) - (medium_severity_count * 0.15) - (len(indicators) * 0.05))
        if not indicators:
            confidence = 0.95
...
class APIValidator:
    ...
    def validate(self, request_json: str) -> Dict:
        ...
        for field in self.FORBIDDEN_FIELDS:
            if field in data:
                return {"valid": False, "error": f"Forbidden field: {field}"}
        ...
        try:
            ...
            extra_fields = set(data.keys()) - expected_fields - {"metadata", "flags", "risk_score", 
                                                                  "sender_name", "suggested_action",
                                                                  "date_range_start", "date_range_end",
                                                                  "duration", "original_intent"}
...
class MainAgent:
    ...
    def process_quarantine_request(self, request: MiddlemanRequest, metadata: RequestMetadata) -> Dict:
...
class RateLimiterWrapper:
    ...
    def check(self, sender: str) -> Dict:
        ...
        result = self._real_limiter.check_message(sender, is_unknown=True)
        
        # Update local counts for test compatibility
        if sender not in self.counts:
            self.counts[sender] = {"minute": 0, "hour": 0, "day": 0}
        
        if result.allowed:
            self._real_limiter.record_message(sender, is_unknown=True)
            self.counts[sender]["minute"] += 1
            self.counts[sender]["hour"] += 1
            self.counts[sender]["day"] += 1
            self.global_count += 1
            return {"allowed": True}
        else:
            return {
                "allowed": False,
                "reason": result.limit_type.value if result.limit_type else "rate_limit",
...
def verified_owner_number() -> str:
    """Bennett's verified phone number."""
    return "+13238776364"
...
def assert_attack_blocked(result: SecurityTestResult, expected_layer: Optional[BlockLayer] = None):
